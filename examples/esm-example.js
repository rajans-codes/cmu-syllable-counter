...
/**
 * ESM (ES Modules) Usage Examples
 * 
 * This file demonstrates how to use the cmu-syllable-counter library
 * in ESM format with import statements.
 * 
 * Run with: node examples/esm-example.js
 */
...
console.log('🎵 CMU Syllable Counter - ESM Examples\n');

// Example 1: Basic syllable counting
console.log('1. Basic Syllable Counting:');
const basicResult = await getSyllableCount('beautiful');
console.log(`"beautiful" has ${basicResult.totalSyllableCount} syllables\n`);

// Example 2: Syllable counting with word details
console.log('2. Syllable Counting with Word Details:');
const detailedResult = await getSyllableCount('hello world', {
  includeHyp: true,
  delimiter: '·',
  includePron: true,
...
const hyphenated = await getHyphenatedString('programming is fun', {
  delimiter: '·',
  includeAnalysis: true
});
...
const poemResult = await getSyllableCount(poem, {
  includeHyp: true,
  includeAnalysis: true
});
